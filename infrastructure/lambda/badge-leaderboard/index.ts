...
// Interfaces
interface BadgeLeaderboardQueryArgs {
    limit?: number;
    nextToken?: string;
}
...
interface BadgeLeaderboardResult {
...
/**
 * Calculate ranks for badge leaderboard entries, handling ties.
 * Users with the same badge count get the same rank.
 */
function calculateRanks(entries: BadgeLeaderboardEntry[], startRank: number = 1): BadgeLeaderboardEntry[] {
...
/**
 * Main handler for badge leaderboard queries.
 */
export const handler = async (event: AppSyncResolverEvent<BadgeLeaderboardQueryArgs>): Promise<BadgeLeaderboardResult> => {
    console.log('Badge leaderboard query:', JSON.stringify(event, null, 2));
    ...
    try {
        ...
        const queryParams: {
            ...
            ScanIndexForward: true, // Ascending order (inverted counts = descending badge counts)
        };
        ...
        // Map to BadgeLeaderboardEntry format
        let entries: BadgeLeaderboardEntry[] = resultItems.map((item) => ({
        ...
        let currentUserEntry: BadgeLeaderboardEntry | undefined;
        ...
        console.error('Error querying badge leaderboard:', error);
...
/**
 * Get the current user's badge leaderboard entry with their rank.
 * Returns undefined if user has no badges.
 */
async function getCurrentUserEntry(userId: string): Promise<BadgeLeaderboardEntry | undefined> {
    // First, get the user's profile to check their badge count
    ...
    if (!user || !user.badgeCount || user.badgeCount === 0) {
        // User has no badges, not on leaderboard
        return undefined;
    }

    // Calculate user's rank by counting how many users have more badges
    const userGSI3SK = calculateGSI3SK(userId, user.badgeCount);
