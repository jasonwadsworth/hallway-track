...
export const handler = async (event: PreTokenGenerationTriggerEvent): Promise<PreTokenGenerationTriggerEvent> => {
  ...
  // Only process external provider sign-ins (Google usernames start with provider name)
  if (!event.userName.includes('_')) {
    console.log('Not an external provider sign-in, skipping');
    return event;
  }
  ...
  if (!email) {
    console.log('No email found, skipping linking');
    return event;
  }
