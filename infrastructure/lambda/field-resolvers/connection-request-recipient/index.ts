...
export const handler = async (event: AppSyncResolverEvent<unknown>): Promise<PublicProfile | null> => {
  console.log('ConnectionRequest.recipient field resolver invoked');
  ...
  if (!recipientUserId) {
    console.warn('No recipientUserId in connection request');
    return null;
  }

  try {
    const users = await batchGetUsers([recipientUserId]);
    const user = users.get(recipientUserId) || null;

    if (!user) {
      console.warn(`User not found for recipientUserId: ${recipientUserId}`);
      return null;
    }

    // Note: Metadata sanitization (hiding initiatorNote and initiatorTags from recipients)
    // is handled at the parent resolver level, not here. This field resolver only
    // returns the public profile of the recipient user.
