...
async function processAllBadgeTypes(userId: string, context: BadgeContext): Promise<void> {
  ...
  try {
    ...
    console.log(`User ${userId} currently has ${currentBadges.length} badges:`, currentBadges.map(b => b.id));
    ...
    for (const [badgeType, evaluator] of Object.entries(badgeEvaluators)) {
      ...
      try {
        ...
        if (badgesToRemove.length > 0) {
          console.log(`Removing ${badgeType} badges:`, badgesToRemove.map(b => b.id));
        ...
        if (badgesToAdd.length > 0) {
          console.log(`Adding ${badgeType} badges:`, badgesToAdd.map(b => b.id));
    ...
    console.log(`Badge processing summary for user ${userId}: +${totalBadgesAdded} -${totalBadgesRemoved} (${currentBadges.length} -> ${updatedBadges.length})`);
...
async function evaluateThresholdBadges(user: User, context: BadgeContext): Promise<BadgeEvaluationResult> {
  ...
  const currentBadges = user.badges || [];
  const currentThresholdBadges = currentBadges.filter(badge =>
    THRESHOLD_BADGES.some(tb => tb.id === badge.id)
  );

  // Determine which badges the user should have based on connection count
  const qualifyingBadges = THRESHOLD_BADGES.filter(tb =>
    user.connectionCount >= tb.threshold
  );
  ...
  const badgesToRemove: Badge[] = currentThresholdBadges.filter(cb =>
    !qualifyingBadges.some(qb => qb.id === cb.id)
  );
    ...
    for (const connection of earlyConnections) {
      try {
        ...
        const hasBadge = supporter.badges?.some(
          (b: Badge) => b.id === 'early-supporter' && b.metadata?.relatedUserId === user.id
        );
        if (hasBadge) {
...
async function evaluateEventBadges(user: User, context: BadgeContext): Promise<BadgeEvaluationResult> {
  ...
  const currentBadges = user.badges || [];
  const hasBadgeForYear = currentBadges.some(
    (b: Badge) => b.id === 'reinvent-connector' && b.metadata?.eventYear === event.year
  );
...
async function updateUserBadges(userId: string, badges: Badge[]): Promise<void> {
  try {
    ...
    console.log(`Successfully updated ${badges.length} badges for user ${userId}`);
  } catch (error) {
