...
export const QRCodeScanner = () => {
        ...
        try {
            ...
            if (devices && devices.length > 0) {
                ...
                if (isMobile) {
                    ...
                    const rearCamera = cameraList.find((camera) => /back|rear|environment/i.test(camera.label));
    ...
    // Extract route info from scanned URL
    const extractRouteInfo = (scannedUrl: string): { type: 'profile' | 'connect'; id: string } | null => {
        try {
            const url = new URL(scannedUrl);
            console.log('Parsed URL pathname:', url.pathname);

            // Check for instant connect URL: /connect/{token}
            // Base64url uses A-Z, a-z, 0-9, - and _ (no padding = in our case)
            const connectMatch = url.pathname.match(/^\/connect\/([A-Za-z0-9_-]+)$/);
            if (connectMatch) {
                console.log('Matched connect URL with token:', connectMatch[1]);
                return { type: 'connect', id: connectMatch[1] };
            }

            // Check for profile URL: /profile/{userId}
            const profileMatch = url.pathname.match(/^\/profile\/([a-zA-Z0-9-]+)$/);
            if (profileMatch) {
                console.log('Matched profile URL with userId:', profileMatch[1]);
            ...
            console.error('Error parsing URL:', scannedUrl, e);
            return null;
        }
    };
    ...
    const onScanSuccess = async (decodedText: string): Promise<void> => {
        ...
        // Extract route info from scanned URL
        const routeInfo = extractRouteInfo(decodedText);

        if (routeInfo) {
            try {
                // Valid HallwayTrak URL detected
                setState((prev) => ({
                    ...prev,
                    scanResult: routeInfo.id,
                    isNavigating: true,
                }));
                ...
                // Determine navigation path based on URL type
                const navigationPath = routeInfo.type === 'connect' ? `/connect/${routeInfo.id}` : `/profile/${routeInfo.id}`;
            ...
            setState((prev) => ({
                ...prev,
                error: "This QR code doesn't appear to be a valid HallwayTrak code. Please scan a HallwayTrak QR code.",
            }));
    ...
    return (
        <div className="qr-scanner-container">
            <div className="qr-scanner-header">
                <h2>Scan QR Code</h2>
                <button className="close-button" onClick={handleClose} aria-label="Close scanner">
                    ✕
                </button>
            </div>
            ...
            <div className="qr-scanner-content">
                ...
                {state.error && (
                    <ErrorMessage
                        ...
                        onDismiss={
                            !state.isScanning && state.cameraId
                                ? () => {
                                      setState((prev) => ({ ...prev, error: null }));
                                      if (state.cameraId) {
                                          startScanning(state.cameraId);
                ...
                {state.isInitializing && !state.error && <LoadingSpinner message="Initializing camera..." />}
                ...
                {state.cameras.length > 1 && state.isScanning && (
                    <>
                        <div className="camera-selector">
                            ...
                            <select id="camera-select" value={state.cameraId || ''} onChange={(e) => switchCamera(e.target.value)} aria-label="Select camera">
